...
const DiagramEditor = () => {
    ...
    const onElementsRemove = useCallback((elementsToRemove: any) => {
        setNodes((nds) => nds.filter((node) => !elementsToRemove.find((el: any) => el.id === node.id)));
        setEdges((eds) => eds.filter((edge) => !elementsToRemove.find((el: any) => el.id === edge.id)));
      }, []);
  
    const onLoad = useCallback((_reactFlowInstance: any) => setReactFlowInstance(_reactFlowInstance), []);
